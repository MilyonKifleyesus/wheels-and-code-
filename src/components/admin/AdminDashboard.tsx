...
const AdminDashboard: React.FC = () => {
  ...
  const avgPrice =
    vehicles.length > 0 ? Math.round(totalValue / vehicles.length) : 0;
  ...
  const recentActivities = [
    ...
    },
    {
      ...
      details:
        bookings.length > 0
          ? `${bookings[bookings.length - 1].service} for ${
              bookings[bookings.length - 1].customerName
            }`
          : "No recent bookings",
      time: "1 hour ago",
      type: "booking",
    ...
    },
    {
      ...
      details:
        vehicles.length > 1
          ? `${vehicles[1].year} ${vehicles[1].make} ${vehicles[1].model}`
          : "No vehicle sales yet",
      time: "1 day ago",
      type: "sale",
      status: "success",
    },
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 mt-2">
            Complete overview of your automotive business
          </p>
        </div>
        <select
          value={timeRange}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.map((metric) => {
          ...
          return (
            <div
              key={metric.label}
              className="bg-dark-graphite border border-gray-800 rounded-lg p-6 hover:border-gray-700 transition-colors duration-300"
            >
        ...
        <div className="bg-dark-graphite border border-gray-800 rounded-lg p-6">
          ...
          <div className="space-y-4">
            {salesData.map((data, index) => (
              ...
              >
                <span className="text-gray-400 w-12 font-medium">
                  {data.month}
                </span>
                <div className="flex-1 mx-4">
                  ...
                  <div className="flex justify-between mt-1 text-xs text-gray-500">
                    ...
                    <span>
                      ${(data.sales + data.services).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-dark-graphite border border-gray-800 rounded-lg p-6">
          ...
          <div className="space-y-4">
            {topServices.map((service, index) => (
              <div
                key={service.name}
                className="flex items-center justify-between p-4 bg-matte-black rounded-sm hover:bg-gray-800 transition-colors duration-300"
              >
          ...
          <div className="mt-4 pt-4 border-t border-gray-800">
            <div className="flex justify-between items-center">
              ...
              <span className="text-acid-yellow font-bold">
                {todayBookings}
              </span>
            </div>
          </div>
        ...
        <div className="space-y-4">
          {validActivities.length > 0 ? (
            validActivities.map((activity, index) => (
              <div
                key={index}
                className="flex items-center space-x-4 p-4 bg-matte-black rounded-sm hover:bg-gray-800 transition-colors duration-300"
              >
                <div
